"""
Build the FINAL Aircast tutorial video with:
  - Frame-synced voiceover (older deeper male voice, each line placed at the
    timestamp where its scene starts on screen)
  - Royalty-free background music (Bensound 'Creative Minds', CC-BY,
    trimmed to 36s with fade in/out, mixed quietly under the voice)
  - Original v10 video as visuals

Output: aircast_tutorial_v10_final.mp4
"""
import asyncio
import edge_tts
import subprocess
...
VIDEO_OUT  = OUT / "aircast_tutorial_v10_final.mp4"

# Older, deeper male voice. Pitch is lowered for bass-y feel.
VOICE = "en-US-GuyNeural"
RATE  = "-4%"
PITCH = "-8Hz"

# Mix levels
VOICE_GAIN  = 1.4    # voice volume multiplier
MUSIC_GAIN  = 0.16   # background music level (~ -16 dB)

# (start_seconds, "phrase")
# Each phrase tightened to fit inside its scene window without overlap.
SCENES = [
    ( 0.4,  "Meet Aircast."),
    ( 2.8,  "Open the app."),
...
async def main():
    ...
    print("\nMuxing voice + music + video...")

    # Inputs: voice clips first (indices 0..N-1), then music (N), then video (N+1)
    inputs = []
    for _, p, _ in clips:
        inputs += ["-i", str(p)]
    ...
    # 1. Delay each voice clip to its scene start
    for i, (start, _, _) in enumerate(clips):
        ms = int(round(start * 1000))
        parts.append(f"[{i}:a]adelay={ms}|{ms}[v{i}]")

    # 2. Mix all voice clips → voice track
    ...
    # 4. Music: take first 36s, fade in/out, set background volume
    parts.append(
    ...
    # 5. Mix voice + music → final audio (voice on top of music)
    # weights=2 1 means voice is weighted higher in the mix
    ...
    print(f"     Background music: Bensound 'Creative Minds' (CC-BY)")
