...
const FINAL_DIR = path.resolve(__dirname, 'video');
(async () => {
  ...
  const url = 'file:///' + path.resolve(__dirname, 'html', 'tutorial_v10.html').replace(/\\/g, '/');
  ...
  if (files.length) {
    ...
    const dst = path.join(FINAL_DIR, 'aircast_tutorial_v10.webm');
